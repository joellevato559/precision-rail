...
let sendEmail, resetEmailContent, PUBLIC_APP_URL;
try {
  ({ sendEmail, resetEmailContent, PUBLIC_APP_URL } = require('../services/email'));
} catch (e) {
  console.warn('[auth] email service unavailable:', e.message);
  PUBLIC_APP_URL = process.env.PUBLIC_APP_URL || '';
  sendEmail = async (msg) => { console.log('[email:fallback]', msg); return { ok: true }; };
  resetEmailContent = ({ name, resetUrl, minutes }) => ({
    subject: 'Password reset',
    text: `Hi ${name}, reset within ${minutes} min: ${resetUrl}`,
